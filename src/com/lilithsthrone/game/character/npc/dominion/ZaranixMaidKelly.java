		...
		public Response getResponse(int responseTab, int index) {
			if(index==1) {
				...
				return new ResponseSex("Use Kelly", "Have some fun with the horny maid.",
						true, false, Main.game.getPlayer(), Main.game.getKelly(), new SMDomStanding(), AFTER_SEX_VICTORY,
						"<p>"
		...
		public Response getResponse(int responseTab, int index) {
			if(index==1) {
				return new ResponseSex("Used", "Kelly uses you.",
						false, false, Main.game.getKelly(), Main.game.getPlayer(), new SMSubStanding(), AFTER_SEX_DEFEAT);
			} else {
				return null;
			}
		}
