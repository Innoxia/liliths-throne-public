...
public class OrificeVaginaUrethra implements OrificeInterface, Serializable {
	...
	public String setWetness(GameCharacter owner, int wetness) {
		...
		if (wetnessChange == 0) {
			if(owner.isPlayer()) {
				...
				return UtilText.parse(owner, "<p style='text-align:center;'>[style.colourDisabled([npc.Name]'s urethral wetness doesn't change...)]</p>");
			}
		}
		...
		if (wetnessChange > 0) {
			if (owner.isPlayer()) {
				...
				return UtilText.parse(owner, 
						"<p>"
							+ "[npc.Name]'s [npc.eyes] widen as [npc.she] feels shudder of excitement run through [npc.her] [npc.pussy+], and [npc.she] realises that [npc.her] urethra has gotten [style.boldGrow(wetter)].</br>"
