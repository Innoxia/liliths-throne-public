 ...
 * @author Innoxia
 */
public class OrificeAnus extends AbstractOrifice implements OrificeInterface, Serializable {
	...
	public OrificeAnus(int wetness, float capacity, int elasticity, int plasticity, boolean virgin, Collection<OrificeModifier> orificeModifiers) {
		...
		ingestionRate = IngestionRate.THREE_FAST.getValue();
	}
	...
	public String setIngestionRate(GameCharacter owner, int ingestionRate) {
		...
		if (ingestionRateChange == 0) {
			if(owner.isPlayer()) {
				return "<p style='text-align:center;'>[style.colourDisabled(Your ingestion rate of fluids taken anally doesn't change...)]</p>";
			} else {
				return UtilText.parse(owner, "<p style='text-align:center;'>[style.colourDisabled([npc.namePos] ingestion rate of fluids taken anally doesn't change...)]</p>");
			}
		}
		...
		if (ingestionRateChange > 0) {
			return UtilText.parse(owner,
					"<p>"
							+ "[npc.Name] [npc.verb(feel)] strange warmth in [npc.her] bowels. "
							+ "[npc.She] [npc.verb(believe)] they should be more absorbent now.<br/>"
							+ "Now [npc.her] igestion rate for fluids taken anally is [style.boldSex(" + ingestionDescriptor + ")]!"
							+ "</p>");

		} else {
			return UtilText.parse(owner,
					"<p>"
							+ "[npc.Name] [npc.verb(feel)] strange cold in [npc.her] bowels. "
