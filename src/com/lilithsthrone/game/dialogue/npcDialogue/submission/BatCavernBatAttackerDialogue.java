...
public class BatCavernBatAttackerDialogue {
	...
	public static final DialogueNode BAT_MORPH_ATTACK = new DialogueNode("Assaulted!", "A bat-morph swoops down from above!", true) {
		...
		public String getContent() {
			if(getBat().getLastTimeEncountered() != -1) {
				if(getBat().isVisiblyPregnant()){
					// Pregnant encounters:
					if(!getBat().isCharacterReactedToPregnancy(Main.game.getPlayer())) {
						...
						return UtilText.parseFromXMLFile("characters/submission/batCavernBat", "ATTACK_REPEAT_PREGNANCY")
									+ (getBat().isAttractedTo(Main.game.getPlayer())
		...
		public Response getResponse(int responseTab, int index) {
			if (index == 1) {
				return new ResponseCombat("Fight", "Stand up for yourself and fight [npc.name]!", getBat()) {
					@Override
					public void effects() {
						if(getBat().isVisiblyPregnant()){
							getBat().setCharacterReactedToPregnancy(Main.game.getPlayer(), true);
						}
					}
				};
				...
				if(getBat().isAttractedTo(Main.game.getPlayer())) {
					return new ResponseSex("Offer body", "Offer your body to [npc.name] so that you can avoid a violent confrontation.",
							...
							true, true,
							new SMGeneric(
									Util.newArrayListOfValues(getBat()),
	...
	public static final DialogueNode AFTER_COMBAT_VICTORY = new DialogueNode("Victory", "", true) {
		...
		@Override
		public String getContent() {
			if(getBat().isAttractedTo(Main.game.getPlayer()) || !Main.game.isNonConEnabled()) {
				return UtilText.parse(getBat(),
						"<p>"
				...
				return UtilText.parse(getBat(),
						...
						+ "</p>"
						+ "<p>"
							+ "[npc.speech(J-Just take my money and leave me alone!)] [npc.she] pleads, throwing [npc.her] "+(getBat().isFeminine()?"purse":"wallet")+" at your feet."
						+ "</p>"
						+ "<p>"
			...
			if(getBat().isAttractedTo(Main.game.getPlayer()) || !Main.game.isNonConEnabled()) {
				if (index == 1) {
					...
					return new ResponseEffectsOnly("Inventory", "Now that you've defeated [npc.name], there's nothing stopping you from helping yourself to [npc.her] clothing and items..."){
						@Override
						public void effects() {
							Main.mainController.openInventory(getBat(), InventoryInteraction.FULL_MANAGEMENT);
						}
					};
	...
	public static final DialogueNode AFTER_COMBAT_DEFEAT = new DialogueNode("Defeat", "", true) {
		...
		@Override
		public String getContent() {
			if(getBat().hasFetish(Fetish.FETISH_TRANSFORMATION_GIVING) && getBat().isWillingToRape(Main.game.getPlayer())) {
				potion = getBat().generateTransformativePotion(Main.game.getPlayer());
				...
				if(potion == null) {
					return UtilText.parse(getBat(),
							...
							+ "<p>"
								+ "[npc.speech(You're my perfect little "
											+getBat().getPreferredBodyDescription("b")
					...
					return UtilText.parse(getBat(),
							...
							+ "<p>"
								...
								+ " [npc.speech(I think you could do with some <i>improvements</i>! I'm going to turn you into my perfect "+getBat().getPreferredBodyDescription("b")+"!)]"
							+ "</p>"
							+ "<p>"
			...
			if(getBat().isAttractedTo(Main.game.getPlayer()) && getBat().isWillingToRape(Main.game.getPlayer())) {
					return UtilText.parse(getBat(),
							"<p>"
								+ "You can't carry on fighting any more, and you feel your [pc.legs] giving out beneath you as you collapse to the ground, defeated."
								+ " A mocking laugh causes you to look up, and you see [npc.name] grinning down at you."
		...
		@Override
		public Response getResponse(int responseTab, int index) {
			if(getBat().hasFetish(Fetish.FETISH_TRANSFORMATION_GIVING) && potion != null && getBat().isWillingToRape(Main.game.getPlayer())) {
				...
				if (index == 1) {
					...
					return new Response("Swallow", "Do as you're told and swallow the strange potion.", AFTER_COMBAT_TRANSFORMATION,
						...
						@Override
						public void effects(){
							Util.Value<String, AbstractItem> potion = getBat().generateTransformativePotion(Main.game.getPlayer());
							Main.game.getTextStartStringBuilder().append(
									...
									+ "</p>"
									+ "<p>"
										+getBat().useItem(potion.getValue(), Main.game.getPlayer(), false)
									+"</p>");
						}
					};
	...
	public static final DialogueNode AFTER_SEX_VICTORY = new DialogueNode("Step back", "", true) {
		...
		public String getContent() {
			if(!getBat().isAttractedTo(Main.game.getPlayer()) && Main.game.isNonConEnabled()) {
				...
				if(Sex.getNumberOfOrgasms(getBat()) >= getBat().getOrgasmsBeforeSatisfied()) {
					return UtilText.parse(getBat(),
							"<p>"
								+ "As you step back from [npc.name], [npc.she] sinks to the floor, totally worn out from [npc.her] orgasm"+(Sex.getNumberOfOrgasms(getBat()) > 1?"s":"")+"."
