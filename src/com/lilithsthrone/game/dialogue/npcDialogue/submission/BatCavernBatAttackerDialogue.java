...
public class BatCavernBatAttackerDialogue {
	...
	public static final DialogueNodeOld AFTER_COMBAT_DEFEAT = new DialogueNodeOld("Defeat", "", true) {
		...
		@Override
		public String getContent() {
			if(Main.game.getActiveNPC().hasFetish(Fetish.FETISH_TRANSFORMATION_GIVING) && Main.game.getActiveNPC().isWillingToRape(Main.game.getPlayer()) && (!Main.game.getActiveNPC().hasFetish(Fetish.FETISH_MASTER) || Main.game.getPlayer().isSlave() )) {
		...
		@Override
		public Response getResponse(int responseTab, int index) {
			if(Main.game.getActiveNPC().hasFetish(Fetish.FETISH_TRANSFORMATION_GIVING) && potion != null && Main.game.getActiveNPC().isWillingToRape(Main.game.getPlayer()) && (!Main.game.getActiveNPC().hasFetish(Fetish.FETISH_MASTER) || Main.game.getPlayer().isSlave()) ) {
				if (index == 1) {
