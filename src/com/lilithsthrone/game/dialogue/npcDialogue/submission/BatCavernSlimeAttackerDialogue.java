...
public class BatCavernSlimeAttackerDialogue {
	...
	public static final DialogueNode SLIME_ATTACK = new DialogueNode("Assaulted!", "A slime lurches forwards from out of the shadows!", true) {
		...
		public String getContent() {
			if(slime().getLastTimeEncountered() != -1) {
				if(slime().isVisiblyPregnant()){
					// Pregnant encounters:
					if(!slime().isCharacterReactedToPregnancy(Main.game.getPlayer())) {
						...
						return UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "ATTACK_REPEAT_PREGNANCY")
									+ (slime().isAttractedTo(Main.game.getPlayer())
		...
		public Response getResponse(int responseTab, int index) {
			if (index == 1) {
				return new ResponseCombat("Fight", "Stand up for yourself and fight [npc.name]!", slime()) {
					@Override
					public void effects() {
						if(slime().isVisiblyPregnant()){
							slime().setCharacterReactedToPregnancy(Main.game.getPlayer(), true);
						}
					}
				};
	...
	public static final DialogueNode AFTER_COMBAT_VICTORY = new DialogueNode("Victory", "", true) {
		...
		public String getContent() {
			if(slime().isAttractedTo(Main.game.getPlayer()) || !Main.game.isNonConEnabled()) {
				return UtilText.parse(slime(), UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "ENEMY_DEFEATED_WANTS_SEX"));
				
			} else {
				return UtilText.parse(slime(), UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "ENEMY_DEFEATED"));
			}
		}
			...
			if (index == 1) {
				...
				if(!slime().isAttractedTo(Main.game.getPlayer()) && !Main.game.isNonConEnabled()) {
					...
					return new ResponseSex("Sex",
							...
							null),
							AFTER_SEX_VICTORY,
							UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_SEX", slime()));
					...
					return new ResponseSex(
							...
							null),
							AFTER_SEX_VICTORY,
							UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_RAPE", slime()));
				}
				...
				if(!slime().isAttractedTo(Main.game.getPlayer()) && !Main.game.isNonConEnabled()) {
					...
					return new ResponseSex("Gentle sex",
							...
							AFTER_SEX_VICTORY,
							UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_SEX_GENTLE", slime()));
					...
					return new ResponseSex("Rape [npc.herHim] (gentle)", "[npc.She] needs to be punished for attacking you like that... (Start the sex scene in the 'gentle' pace.)",
							...
							new SMGeneric(
									...
									ResponseTag.START_PACE_PLAYER_DOM_GENTLE),
							AFTER_SEX_VICTORY,
							UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_RAPE_GENTLE", slime()));
				}
				...
				if(!slime().isAttractedTo(Main.game.getPlayer()) && !Main.game.isNonConEnabled()) {
					...
					return new ResponseSex("Rough sex",
							...
							AFTER_SEX_VICTORY,
							UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_SEX_ROUGH", slime()));
					...
					return new ResponseSex("Rape [npc.herHim] (rough)", "[npc.She] needs to be punished for attacking you like that... (Start the sex scene in the 'rough' pace.)",
							...
							new SMGeneric(
									...
									null,
									ResponseTag.START_PACE_PLAYER_DOM_ROUGH),
							AFTER_SEX_VICTORY, UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_RAPE_ROUGH", slime()));
				}
				...
				if(!slime().isAttractedTo(Main.game.getPlayer())) {
					...
					return new ResponseSex("Submit",
							...
							null),
							AFTER_SEX_DEFEAT,
							UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_SEX_SUBMIT", slime()));
				}
...
//			} 
				...
				return new Response(
					...
					@Override
					public void effects() {
						Main.game.getTextStartStringBuilder().append(UtilText.parseFromXMLFile("characters/submission/batCavernSlime", "AFTER_COMBAT_VICTORY_BANISH_NPC", slime()));
						Main.game.banishNPC(slime());
					}
				};
	...
	public static final DialogueNode AFTER_COMBAT_DEFEAT = new DialogueNode("Defeat", "", true) {
		...
		@Override
		public String getContent() {
			if(slime().hasFetish(Fetish.FETISH_TRANSFORMATION_GIVING) && slime().isWillingToRape(Main.game.getPlayer())) {
				potion = slime().generateTransformativePotion(Main.game.getPlayer());
				
				if(potion == null) {
					return UtilText.parse(slime(),
							"<p>"
							...
							+ "</p>"
							+ "<p>"
								+ "[npc.speech(You're my perfect little " +slime().getPreferredBodyDescription("b") + " now! Don't forget, bitch, <i>I'm</i> the one in charge!)] [npc.she] growls, before pulling you into a forceful kiss."
							+ "</p>");
					...
					return UtilText.parse(slime(),
							...
							+ "<p>"
								...
								+ " [npc.speech(I think you could do with some <i>improvements</i>! I'm going to turn you into my perfect "+slime().getPreferredBodyDescription("b")+"!)]"
							+ "</p>"
							+ "<p>"
			...
			if(slime().isAttractedTo(Main.game.getPlayer()) && slime().isWillingToRape(Main.game.getPlayer())) {
					return UtilText.parse(slime(),
							"<p>"
								+ "You can't carry on fighting any more, and you feel your [pc.legs] giving out beneath you as you collapse to the ground, defeated."
								+ " A mocking laugh causes you to look up, and you see [npc.name] grinning down at you."
		...
		@Override
		public Response getResponse(int responseTab, int index) {
			if(slime().hasFetish(Fetish.FETISH_TRANSFORMATION_GIVING) && potion != null && slime().isWillingToRape(Main.game.getPlayer())) {
				if (index == 1) {
					...
					return new Response("Swallow", "Do as you're told and swallow the strange potion.", AFTER_COMBAT_TRANSFORMATION,
						...
						@Override
						public void effects(){
							Util.Value<String, AbstractItem> potion = slime().generateTransformativePotion(Main.game.getPlayer());
							Main.game.getTextStartStringBuilder().append(
									...
									+ "</p>"
									+ "<p>"
										+slime().useItem(potion.getValue(), Main.game.getPlayer(), false)
									+"</p>");
						}
					};
			...
			if(slime().isAttractedTo(Main.game.getPlayer())) {
				return UtilText.parse(slime(),
						...
						+ "<p>"
							...
							+slime().getPreferredBodyDescription("b")
