...
public class MiscDialogue {
	...
	private static String getUpgradeEntry(Cell cell, PlaceUpgrade upgrade) {
		...
		if(!canBuy) {
			miscDialogueSB.append("<p>"
				+ "<i>"
				+ "[style.colourBad("+ getPurchaseAvailabilityTooltipText(MiscDialogue.cellToInspect.getPlace(), upgrade)+")]"
				+"</i>"
			+ "</p>");
		}
	...
	private static String getSlaveInformationHeader(NPC character) {
		...
		headerSB.append(
				"<div class='container-full-width' style='text-align:center;'>"
//						+ "<h6 style='color:"+Colour.GENERIC_EXCELLENT.toWebHexString()+"; text-align:center;'>Slave Overview</h6>"
	...
	public static final DialogueNodeOld SLAVE_MANAGEMENT_JOBS = new DialogueNodeOld("Slave Management", ".", true) {
		...
		public String getContent() {
			...
			for(SlaveJob job : SlaveJob.values()) {
				affectionChange = job.getAffectionGain();
				obedienceChange = job.getObedienceGain();
