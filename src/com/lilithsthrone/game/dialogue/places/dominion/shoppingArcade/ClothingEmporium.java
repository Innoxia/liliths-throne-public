						...
						if(Main.game.getNpc(Nyan.class).getAffection(Main.game.getPlayer())<95) {
							...
							return new ResponseSex("Sex",
									...
									new SMMissionary(
											...
											Util.newHashMapOfValues(new Value<>(Main.game.getNpc(Nyan.class), SexPositionSlot.MISSIONARY_ON_BACK))),
									null,
		...
		public Response getResponse(int responseTab, int index) {
			if (index == 0) {
				return new Response("Leave", "Exit the store.", EXTERIOR) {
					@Override
					public void effects() {
						Main.game.getPlayer().addItem(AbstractItemType.generateItem(ItemType.getSpellScrollType(SpellSchool.EARTH)), false, true);
