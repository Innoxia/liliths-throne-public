...
public class DomDoggy {
	...
	public static SexAction MUTUAL_DOGGY_CREAMPIE_ORGASM = new SexAction(
		...
		@Override
		public String getActionDescription() {
			return "Fill [npc.name]'s ass with your cum.";
		}
		...
		public String getDescription() {
			...
			if(Sex.getPartner().hasPenis()) {
				UtilText.nodeContentSB.append("</br></br>"
						+ "[npc.Name] suddenly lets out [npc.a_moan+]; [npc.her] arousal proving to be too much for [npc.herHim] to bear as [npc.she] orgasms.");
				UtilText.nodeContentSB.append(" As [npc.her] [npc.balls+] tense up");
				switch (Sex.getPartner().getPenisCumProduction()) {
					...
					case TWO_SMALL_AMOUNT:
						...
						break;
					case THREE_AVERAGE:
						UtilText.nodeContentSB.append(", [npc.her] [npc.cum+] squirts");
						break;
					case FOUR_LARGE:
						UtilText.nodeContentSB.append(", [npc.her] [npc.cum+] shoots");
						break;
					case FIVE_HUGE:
						UtilText.nodeContentSB.append(", [npc.her] [npc.cum+] pours");
						break;
					case SIX_EXTREME:
						UtilText.nodeContentSB.append(", [npc.her] [npc.cum+] pours");
						break;
				...
				if(Sex.getPartner().getPenisCumProduction()!=CumProduction.ZERO_NONE){
					if(Sex.getPartner().isWearingCondom()) {
						UtilText.nodeContentSB.append("out into the condom that [npc.she]'s wearing.");
						
					} else if (!Sex.getPartner().isCoverableAreaExposed(CoverableArea.PENIS)) {
						UtilText.nodeContentSB.append(" into [npc.her] [npc.lowClothing(penis)].");
						
					} else {
						UtilText.nodeContentSB.append(" out all over the floor.");
					}
				}
			}
			if(Sex.getPartner().hasVagina()) {
				UtilText.nodeContentSB.append("</br>"
						+ "[npc.She] suddenly lets out a manic squeal as [npc.her] orgasm starts rising up from [npc.her] [npc.pussy+], and, with trembling legs, [npc.her] female climax crashes over [npc.herHim].");
			}
	...
	public static SexAction MUTUAL_DOGGY_CREAMPIE_ORGASM_PUSSY = new SexAction(
		...
		public String getDescription() {
			...
			if(Main.game.getPlayer().isWearingCondom()) {
				UtilText.nodeContentSB.append(" [npc.Name] [npc.moansVerb+] as [npc.her] orgasm crashes over [npc.herHim], and you feel [npc.her] [npc.pussy+] clenching down around your [pc.cock]."
						+ " As your [pc.balls+] tense up");
				switch (Main.game.getPlayer().getPenisCumProduction()) {
					case ZERO_NONE:
						UtilText.nodeContentSB.append(", you realise that you're not able to produce even one drop of cum.");
						break;
