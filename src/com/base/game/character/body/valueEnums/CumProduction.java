...
public enum CumProduction {
	...
	public static CumProduction getCumProductionFromInt(int cumProduction) {
		if (cumProduction == 0)
			return ZERO_NONE;
		else if (cumProduction <= ONE_TRICKLE.maximumValue)
			return ONE_TRICKLE;
		else if (cumProduction <= TWO_SMALL_AMOUNT.maximumValue)
			return TWO_SMALL_AMOUNT;
		else if (cumProduction <= THREE_AVERAGE.maximumValue)
			return THREE_AVERAGE;
		else if (cumProduction <= FOUR_LARGE.maximumValue)
			return FOUR_LARGE;
		else if (cumProduction <= FIVE_HUGE.maximumValue)
			return FIVE_HUGE;
		else if (cumProduction <= SIX_EXTREME.maximumValue)
			return SIX_EXTREME;
		else
