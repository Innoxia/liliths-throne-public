...
public enum MakeupLevel {
	NONE("no",
			0),
	MINIMAL("an unnoticable amount of",
			1),
	LOW("a slight touch of",
			2),
	AVERAGE("a tasteful amount of",
			3),
	HIGH("a lot of",
			4),
	VERY_HIGH("a heavy coating of",
			5),
	EXTREME("a thick plastering of",
			6),
	MAXIMUM("an absurd amount of",
			7);
	...
	public static MakeupLevel getMakeupLevelFromInt(int makeupLevel) {
		for (MakeupLevel cs : values())
			if (makeupLevel <= cs.makeupLevel)
				return cs;
		return MAXIMUM;
	}
