...
public enum RenderingEngine {
	...
	if (Main.mainController.getWebEngineInventory() != null) {
	    ...
	    InventorySlot[] inventorySlots = { InventorySlot.EYES, InventorySlot.HEAD, InventorySlot.MOUTH,
		    InventorySlot.NECK, InventorySlot.TORSO, InventorySlot.CHEST, InventorySlot.WRIST,
		    InventorySlot.STOMACH, InventorySlot.HAND, InventorySlot.FINGER, InventorySlot.LEG,
		    InventorySlot.GROIN, InventorySlot.ANKLE, InventorySlot.FOOT, InventorySlot.SOCK };
	    int counter = 1;
		...
		if (clothing != null) {
		    ...
		    inventorySB.append(
			    ...
			    "<div class='equipSlot" + " " + (clothing.getRarity().getName()) + "'"
				    ...
				    + (!clothing.getDisplacedList().isEmpty() ? "<div class='displacedIcon'>"
					    + SVGImages.SVG_IMAGE_PROVIDER.getDisplacedIcon() + "</div>" : "")
				    // If clothing is cummed in:
				    + (clothing.isDirty()
					    ? "<div class='cummedIcon'>"
						    + SVGImages.SVG_IMAGE_PROVIDER.getCummedInIcon() + "</div>"
					    : "")
				    // If clothing is too masculine:
				    + (clothing.getClothingType().getFemininityMaximum() < charactersInventoryToRender
					    .getFemininity() ? "<div class='femininityIcon'>"
						    + SVGImages.SVG_IMAGE_PROVIDER.getMasculineWarningIcon() + "</div>"
						    : "")
				    // If clothing is too feminine:
				    + (clothing.getClothingType().getFemininityMinimum() > charactersInventoryToRender
					    .getFemininity() ? "<div class='femininityIcon'>"
						    + SVGImages.SVG_IMAGE_PROVIDER.getFeminineWarningIcon() + "</div>"
						    : "")
			...
			inventorySB.append("<div class='equipSlot'><div class='overlay disabled' id='"
				+ invSlot.toString() + "Slot'></div></div>");
		    else if (ClothingType.slotBlockedByRace(charactersInventoryToRender, invSlot) != null)
			inventorySB
				.append("<div class='equipSlot'>" + "<div class='overlay disabled' id='"
					+ invSlot.toString() + "Slot'></div>" + "<div class='raceBlockIcon'>"
					+ ClothingType.slotBlockedByRace(charactersInventoryToRender, invSlot)
						.getStatusEffect().getSVGString(charactersInventoryToRender)
					+ "</div>" + "</div>");
		    else
		...
		if (charactersInventoryToRender.getMainWeapon() != null) {
		    inventorySB.append("<div class='equipSlot weapon "
			    + charactersInventoryToRender.getMainWeapon().getRarity().getName() + "'>"
			    + charactersInventoryToRender.getMainWeapon().getSVGString() + "<div class='overlay' id='"
			    + InventorySlot.WEAPON_MAIN.toString() + "Slot'></div>" + "</div>");
		} else {
		    inventorySB.append("<div class='equipSlot weapon' id='" + InventorySlot.WEAPON_MAIN.toString()
			    + "Slot'></div>");
		}
	    } else {
		inventorySB.append(
			"<div class='equipSlot weapon' id='" + InventorySlot.WEAPON_MAIN.toString() + "Slot'></div>");
	    }
	    // Offhand weapon:
	    if (charactersInventoryToRender != null) {
	    ...
	    InventorySlot[] piercingSlots = { InventorySlot.PIERCING_EAR, InventorySlot.PIERCING_NOSE,
		    InventorySlot.PIERCING_TONGUE, InventorySlot.PIERCING_LIP, InventorySlot.PIERCING_NIPPLE,
		    InventorySlot.PIERCING_STOMACH, InventorySlot.PIERCING_VAGINA, InventorySlot.PIERCING_PENIS };
	    ...
	    inventorySB.append(
		    "<div class='equipSlot small' id='protectionSlot'>" + (charactersInventoryToRender.isWearingCondom()
			    ? SVGImages.SVG_IMAGE_PROVIDER.getProtectionEnabled()
			    : SVGImages.SVG_IMAGE_PROVIDER.getProtectionDisabled()) + "</div>");
	    inventorySB.append("<div class='equipSlot small' id='tattooSlot'>"
		    + SVGImages.SVG_IMAGE_PROVIDER.getTattoo() + "</div>");
	    inventorySB.append("</section>");
    ...
    private static float renderedStrengthValue = 0, renderedIntelligenceValue = 0, renderedFitnessValue = 0,
	    renderedHealthValue = 0, renderedManaValue = 0, renderedStaminaValue = 0, renderedPlayerCorruptionValue = 0,
	    renderedPartnerCorruptionValue = 0, renderedPlayerArousalValue = 0, renderedPartnerArousalValue = 0;
	...
	uiAttributeSB
		.append("<body>" + "<div class='attribute-container'>"
			...
			+ "</p>" + "<div class='overlay' id='EXTRA_ATTRIBUTES'></div>" + "</div>"
			...
			+ "</b>" + "</p>" + "<div class='barBackgroundExp'>"
	...
	if (Main.game.isInSex()) {
	    ...
	    uiAttributeSB.append("<div class='attribute-container'>"
		    ...
		    + ArousalLevel.getArousalLevelFromValue(Main.game.getPlayer().getArousal()).getRelatedStatusEffect()
			    .getSVGString(Main.game.getPlayer())
		    ...
		    // Corruption:
		    + "<div class='full-width-container' style='padding:0;'>" + "<div class='statusEffectBackground'>"
		    + CorruptionLevel
			    .getCorruptionLevelFromValue(Main.game.getPlayer().getAttributeValue(Attribute.CORRUPTION))
		...
		uiAttributeSB.append("<div class='statusEffectBackground'>" + f.getSVGString()
			+ "<div class='overlay' id='FETISH_PLAYER_" + f + "'></div>" + "</div>");
		hasStatusEffects = true;
	    }

	    for (StatusEffect se : Main.game.getPlayer().getStatusEffects()) {
		if (se.isSexEffect()) {
		    uiAttributeSB.append("<div class='statusEffectBackground'>" + se.getSVGString(Main.game.getPlayer())
			    + "<div class='overlay' id='SE_PLAYER_" + se + "'></div>" + "</div>");
		    hasStatusEffects = true;
		}
	    }
	    if (!hasStatusEffects) {
	    ...
	    uiAttributeSB.append("</div>"
		    ...
		    + "</b>" + "</p>" + "<div class='overlay' id='PARTNER_" + Attribute.EXPERIENCE.getName()
		    + "'></div>" + "</div>"
		    ...
		    + "<div class='full-width-container' style='margin:8 0 0 0; margin:0; padding:0;'>"
		    + "<div class='statusEffectBackground'>"
		    + ArousalLevel.getArousalLevelFromValue(Sex.getPartner().getArousal()).getRelatedStatusEffect()
		    ...
		    + "<div class='full-width-container' style='padding:0;'>" + "<div class='statusEffectBackground'>"
		    + CorruptionLevel
			    .getCorruptionLevelFromValue(Sex.getPartner().getAttributeValue(Attribute.CORRUPTION))
		...
		uiAttributeSB.append("<div class='statusEffectBackground'>" + f.getSVGString()
			+ "<div class='overlay' id='FETISH_PARTNER_" + f + "'></div>" + "</div>");
		hasStatusEffects = true;
	    }

	    for (StatusEffect se : Sex.getPartner().getStatusEffects()) {
		if (se.isSexEffect()) {
		    uiAttributeSB.append("<div class='statusEffectBackground'>" + se.getSVGString(Sex.getPartner())
			    + "<div class='overlay' id='SE_PARTNER_" + se + "'></div>" + "</div>");
		    hasStatusEffects = true;
		}
	    }
	    if (!hasStatusEffects) {
		...
		uiAttributeSB.append("<div class='attribute-container'>"
			+ "<p style='text-align:center;padding:0;margin:0;'><b>Attributes</b></p>"
			+ "<div class='quarter-width-container'>" + "<div class='imageIcon'>"
			+ StrengthLevel
				.getStrengthLevelFromValue(Main.game.getPlayer().getAttributeValue(Attribute.STRENGTH))
				.getRelatedStatusEffect().getSVGString(Main.game.getPlayer())
	    ...
	    uiAttributeSB.append("<div class='combat-attributes'>"
		    ...
		    + Attribute.HEALTH_MAXIMUM.getSVGString() + "</div>" + "<div class='barBackgroundAtt'>"
		    + "<div style='width:"
		    ...
		    + "<div class='full-width-container' style='margin:0;padding:0;'>" + "<div class='imageIcon'>"
		    + Attribute.MANA_MAXIMUM.getSVGString() + "</div>" + "<div class='barBackgroundAtt'>"
		    + "<div style='width:"
	    ...
	    // Infinite duration:
	    for (StatusEffect se : Main.game.getPlayer().getStatusEffects()) {
		if (!se.isCombatEffect() && Main.game.getPlayer().getStatusEffectDuration(se) == -1
			&& se.renderInEffectsPanel())
		    uiAttributeSB.append("<div class='statusEffectBackground"
			    + (se.isCombatEffect() && !se.isBeneficial() ? " negativeCombat" : "")
			    + (se.isCombatEffect() && se.isBeneficial() ? " positiveCombat" : "") + "'>"
			    + se.getSVGString(Main.game.getPlayer()) + "<div class='overlay' id='SE_PLAYER_" + se
			    + "'></div>" + "</div>");
	    }
	    // Timed:
	    for (StatusEffect se : Main.game.getPlayer().getStatusEffects()) {
	...
	if (Main.mainController != null)
		...
		if (!Main.game.getCurrentDialogueNode().isNoTextForContinuesDialogue()
	...
	for (int y = playerPosition.getY() + mapSize; y >= playerPosition.getY() - mapSize; y--) {
		...
		if (x < Main.game.getActiveWorld().WORLD_WIDTH && x >= 0 && y < Main.game.getActiveWorld().WORLD_HEIGHT
			&& y >= 0) {// If within bounds of map:

		    if (Main.game.getActiveWorld().getCell(x, y).isDiscovered() || Main.game.isDebugMode()) { // If the
													      // tile is
													      // discovered:

			if (Main.game.getActiveWorld().getCell(x, y).getPlace() == GenericPlace.IMPASSABLE) {
			    mapSB.append("<div class='map-tile blank' style='width:" + (4 * unit) + "%; height:"
				    + (4 * unit) + "%;'></div>");
			} else {

			    // This is the "move North" tile:
			    if (y == playerPosition.getY() + 1 && x == playerPosition.getX()
				    && Main.game.getActiveWorld().getCell(x, y).getPlace() != GenericPlace.IMPASSABLE) {
				if (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()) {
				    mapSB.append("<div class='map-tile movement dangerous' id='upButton' style='width:"
					    + (4 * unit - borderSizeReduction) + "%; height:"
					    + (4 * unit - borderSizeReduction) + "%; border-width:1%;'>");

				} else {
				    mapSB.append("<div class='map-tile movement' id='upButton' style='width:"
					    + (4 * unit - borderSizeReduction) + "%; height:"
					    + (4 * unit - borderSizeReduction) + "%; border-width:1%; border-color:"
					    + (Main.game.getPlayer().getFemininity() <= Femininity.MASCULINE
						    .getMaximumFemininity()
				...
				if (Main.game.getActiveWorld().getCell(x, y).getPlace().getSVGString() != null)
				    mapSB.append("<div class='place-icon'>"
					    + Main.game.getActiveWorld().getCell(x, y).getPlace().getSVGString()
					    + "</div>");

				mapSB.append("<b class='hotkey-icon"
					+ (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()
						? " dangerous"
						: "")
					+ "'>"
					+ (Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_NORTH) == null
						? ""
						: Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_NORTH)
							.getName())
					+ "</b>");
			    ...
			    } else if (y == playerPosition.getY() - 1 && x == playerPosition.getX()
				    && Main.game.getActiveWorld().getCell(x, y).getPlace() != GenericPlace.IMPASSABLE) {
				if (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()) {
				    mapSB.append(
					    "<div class='map-tile movement dangerous' id='downButton' style='width:"
						    + (4 * unit - borderSizeReduction) + "%; height:"
						    + (4 * unit - borderSizeReduction) + "%; border-width:1%;'>");

				} else {
				    mapSB.append("<div class='map-tile movement' id='downButton' style='width:"
					    + (4 * unit - borderSizeReduction) + "%; height:"
					    + (4 * unit - borderSizeReduction) + "%; border-width:1%; border-color:"
					    + (Main.game.getPlayer().getFemininity() <= Femininity.MASCULINE
						    .getMaximumFemininity()
				...
				mapSB.append("<b class='hotkey-icon"
					+ (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()
						...
						: "")
					+ "'>"
					+ (Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_SOUTH) == null
						? ""
						: Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_SOUTH)
							.getName())
					+ "</b>");
			    ...
			    } else if (y == playerPosition.getY() && x == playerPosition.getX() - 1
				    && Main.game.getActiveWorld().getCell(x, y).getPlace() != GenericPlace.IMPASSABLE) {
				if (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()) {
				    mapSB.append(
					    "<div class='map-tile movement dangerous' id='leftButton' style='width:"
						    + (4 * unit - borderSizeReduction) + "%; height:"
						    + (4 * unit - borderSizeReduction) + "%; border-width:1%;'>");

				} else {
				    mapSB.append("<div class='map-tile movement' id='leftButton' style='width:"
					    + (4 * unit - borderSizeReduction) + "%; height:"
					    + (4 * unit - borderSizeReduction) + "%; border-width:1%; border-color:"
					    + (Main.game.getPlayer().getFemininity() <= Femininity.MASCULINE
						    .getMaximumFemininity()
				...
				mapSB.append("<b class='hotkey-icon"
					+ (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()
						...
						: "")
					+ "'>"
					+ (Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_WEST) == null
						? ""
						: Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_WEST)
							.getName())
					+ "</b>");
				...
				if (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()) {
				    mapSB.append(
					    "<div class='map-tile movement dangerous' id='rightButton' style='width:"
						    + (4 * unit - borderSizeReduction) + "%; height:"
						    + (4 * unit - borderSizeReduction) + "%; border-width:1%;'>");

				} else {
				    mapSB.append("<div class='map-tile movement' id='rightButton' style='width:"
					    + (4 * unit - borderSizeReduction) + "%; height:"
					    + (4 * unit - borderSizeReduction) + "%; border-width:1%; border-color:"
					    + (Main.game.getPlayer().getFemininity() <= Femininity.MASCULINE
						    .getMaximumFemininity()
				...
				mapSB.append("<b class='hotkey-icon"
					+ (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()
						...
						: "")
					+ "'>"
					+ (Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_EAST) == null
						? ""
						: Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MOVE_EAST)
							.getName())
					+ "</b>");
				...
				mapSB.append("<div class='map-tile"
					+ (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()
						? (y == playerPosition.getY() && x == playerPosition.getX()
							? " player dangerous"
							: " dangerous")
						: (y == playerPosition.getY() && x == playerPosition.getX() ? " player"
							: ""))
				...
				if (Main.game.getActiveWorld().getCell(x, y).getPlace().getSVGString() != null) {
					...
					if (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()) {
					    mapSB.append(
						    "<div class='place-icon' style='margin:7%;width:86%;height:86%;'>"
							    + SVGImages.SVG_IMAGE_PROVIDER
								    .getPlayerPlaceMapDangerousIcon()
							    + "</div>");
						...
						mapSB.append(
							"<div class='place-icon' style='margin:7%;width:86%;height:86%;'>"
								+ SVGImages.SVG_IMAGE_PROVIDER
									.getPlayerPlaceMapIconMasculine()
								+ "</div>");

					    } else if (Main.game.getPlayer().getFemininity() <= Femininity.ANDROGYNOUS
						    .getMaximumFemininity()) {
						mapSB.append(
							"<div class='place-icon' style='margin:7%;width:86%;height:86%;'>"
								+ SVGImages.SVG_IMAGE_PROVIDER
									.getPlayerPlaceMapIconAndrogynous()
								+ "</div>");
						...
						mapSB.append(
							"<div class='place-icon' style='margin:7%;width:86%;height:86%;'>"
								+ SVGImages.SVG_IMAGE_PROVIDER
									.getPlayerPlaceMapIconFeminine()
								+ "</div>");
					    }
					}
				    }
				    mapSB.append("<div class='place-icon' style='margin:18%;width:64%;height:64%;'>"
					    + Main.game.getActiveWorld().getCell(x, y).getPlace().getSVGString()
					    + "</div>");
				    ...
				    if (Main.game.getActiveWorld().getCell(x, y).getPlace().isDangerous()) {
					mapSB.append("<div class='place-icon' style='margin:18%;width:64%;height:64%;'>"
						+ SVGImages.SVG_IMAGE_PROVIDER.getPlayerMapDangerousIcon() + "</div>");
				    } else {
					if (Main.game.getPlayer().getFemininity() <= Femininity.MASCULINE
						.getMaximumFemininity()) {
					    mapSB.append(
						    "<div class='place-icon' style='margin:18%;width:64%;height:64%;'>"
							    + SVGImages.SVG_IMAGE_PROVIDER.getPlayerMapIconMasculine()
							    + "</div>");
					    ...
					    mapSB.append(
						    "<div class='place-icon' style='margin:18%;width:64%;height:64%;'>"
							    + SVGImages.SVG_IMAGE_PROVIDER.getPlayerMapIconAndrogynous()
							    + "</div>");

					} else {
					    mapSB.append(
						    "<div class='place-icon' style='margin:18%;width:64%;height:64%;'>"
							    + SVGImages.SVG_IMAGE_PROVIDER.getPlayerMapIconFeminine()
							    + "</div>");
					}
				    }
				}
		    ...
		    mapSB.append("<div class='map-tile blank' style='width:" + (4 * unit) + "%; height:" + (4 * unit)
			    + "%;'></div>");
		}
	    }
	...
	for (int i = mapIcons.size(); i > 0; i--) {
	    mapSB.append("<div class='npc-icon' style='left:" + (((i - 1) * 6) + 2) + "px;'>" + mapIcons.get(i - 1)
		    + "</div>");
	}
    }
	...
	switch (display) {
	case NORMAL:
	case STATUS_EFFECT_MESSAGE:
	    Main.mainController.getWebViewInventory().setVisible(false);
	    Main.mainController.getWebViewMap().setVisible(true);
	    Main.mainController.setMapTitleContent(weatherBar((Util.capitaliseSentence(
		    Main.game.getActiveWorld().getCell(Main.game.getPlayer().getLocation()).getPlace().getName()))));
	    break;

	case INVENTORY:
	    Main.mainController.getWebViewInventory().setVisible(true);
	    Main.mainController.getWebViewMap().setVisible(false);
	    Main.mainController.setMapTitleContent(weatherBar(
		    (RenderingEngine.ENGINE.getCharactersInventoryToRender().isPlayer() ? "Your Equipped Items"
			    : Util.capitaliseSentence(
				    RenderingEngine.ENGINE.getCharactersInventoryToRender().getName()))));
	...
	case CHARACTERS_PRESENT:
	    ...
	    Main.mainController.setMapTitleContent(
		    weatherBar((RenderingEngine.ENGINE.getCharactersInventoryToRender() == null ? "Examining characters"
			    : Util.capitaliseSentence(
				    RenderingEngine.ENGINE.getCharactersInventoryToRender().getName()))));
	    break;
	default:
	...
	return "<body class='weatherBodyTitle'>"
		// + "<div class='weather-image'>"
		// + Main.game.getWeatherImage()
		// + "<div class='overlay' id='weather'></div>"
		// + "</div>"
		+ "<p style='text-align:center;margin:0;padding:0;width:75vw;float:left;'>" + "<b>"
		+ (Main.game.isDayTime() ? "Day " : "Night ") + Main.game.getDayNumber() + ", "
	...
	Main.mainController.setButtonsContent("<div class='quarterContainer'>" + "<div class='button"
		+ (!Main.game.getCurrentDialogueNode().isOptionsDisabled() ? "" : " disabled") + "' id='mainMenu'>"
		+ SVGImages.SVG_IMAGE_PROVIDER.getMenuIcon()
		+ (!Main.game.getCurrentDialogueNode().isOptionsDisabled() ? "" : "<div class='disabledLayer'></div>")
		+ "</div>" + "</div>"

		+ "<div class='quarterContainer'>" + "<div class='button"
		+ (Main.game.getPlayer().isMainQuestUpdated() || Main.game.getPlayer().isSideQuestUpdated()
			...
			|| Main.game.getPlayer().isNewItemDiscovered() || Main.game.getPlayer().isNewRaceDiscovered()
			|| Main.game.getPlayer().getPerkPoints() > 0 || Main.game.getPlayer().getLevelUpPoints() > 0
				? " highlight"
				: "")
		+ (!Main.game.getCurrentDialogueNode().isOptionsDisabled() ? "" : " disabled") + "' id='journal'>"
		+ SVGImages.SVG_IMAGE_PROVIDER.getJournalIcon()
		+ (!Main.game.getCurrentDialogueNode().isOptionsDisabled() ? "" : "<div class='disabledLayer'></div>")
		+ "</div>" + "</div>"

		+ "<div class='quarterContainer'>" + "<div class='button"
		+ (Main.game.getActiveWorld().getCell(Main.game.getPlayer().getLocation()).getInventory()
			.getInventorySlotsTaken() > 0 ? " highlight" : "")
		+ (Main.mainController.isInventoryDisabled() ? " disabled" : "") + "' id='inventory'>"
		+ ((Main.game.isInCombat() || Main.game.isInSex())
			&& Main.game.getCurrentDialogueNode().getMapDisplay() != MapDisplay.CHARACTERS_PRESENT
				? (RenderingEngine.ENGINE.getCharactersInventoryToRender().isPlayer()
					? SVGImages.SVG_IMAGE_PROVIDER.getInventorySwitchIcon()
					: SVGImages.SVG_IMAGE_PROVIDER.getInventorySwitchOppositeIcon())
				: SVGImages.SVG_IMAGE_PROVIDER.getInventoryIcon())
		+ (Main.mainController.isInventoryDisabled() ? "<div class='disabledLayer'></div>" : "") + "</div>"
		+ "</div>"
		...
		+ "<div class='quarterContainer'>" + "<div class='button"
		+ (Main.game.getCurrentDialogueNode().isTravelDisabled() ? " disabled" : "") + "' id='mapZoom'>"
		+ (RenderingEngine.isZoomedIn() ? SVGImages.SVG_IMAGE_PROVIDER.getZoomOutIcon()
			: SVGImages.SVG_IMAGE_PROVIDER.getZoomInIcon())
